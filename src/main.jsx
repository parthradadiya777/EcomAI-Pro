...
function MarketplaceConnection({state}){
  ...
  return <>
    ...
    <section className="flow-card"><div><span className="eyebrow">WHAT HAPPENS NEXT</span><h3>Connection is only the first layer</h3><p>After marketplace connection, continue directly to product research.</p></div><div className="flow-steps"><span className="done">1 Marketplace</span><b>→</b><span>2 Product / Listing</span><b>→</b><span>3 Competitor & Trends</span><b>→</b><span>4 What Should I Create?</span></div></section>
...
function MarketPlaceholder({onBack,product,setModule}){
  ...
  return <div className="content">
    <ModuleHeader title="Competitor & Market Analysis" sub={`${product?.platform||"Marketplace"} research for ${product?.title||"selected product"}. Findings below are based on the verified references you selected.`}/>
...
function ListingAI({product,onBack}){
  ...
  return <div className="content">
    <ModuleHeader title="Listing AI" sub={`${p.platform||"Marketplace"} listing draft built from your product profile, verified competitor references and keyword research.`}/>
    ...
    <section className="listing-ai-hero"><div><span className="eyebrow">LISTING BUILDER</span><h2>Create a marketplace-specific listing</h2><p>EcomAI keeps the listing tied to the selected marketplace and the evidence collected in product research. It does not invent missing product specifications.</p></div><div className="listing-platform"><Globe2 size={18}/><b>{p.platform||"Marketplace"}</b><small>Platform-specific workflow</small></div></section>
