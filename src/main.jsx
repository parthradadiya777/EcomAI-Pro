...
function ListingAI({product,onBack}){
  ...
  const onZip=async(e)=>{
    const file=e.target.files?.[0];if(!file)return;
    setError("");setStatus("Reading Product Images ZIP…");
    try{
    ...
    }catch(e){setError(e?.message||"Could not read the image ZIP.");setStatus("")}
  };
