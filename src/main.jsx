...
function ListingAI({product,onBack}){
  ...
  const parseZip=async(file)=>{
    ...
    const imageMime=name=>{
      ...
      const m={jpg:"image/jpeg",jpeg:"image/jpeg",png:"image/png",webp:"image/webp",gif:"image/gif",bmp:"image/bmp",avif:"image/avif",heic:"image/heic",heif:"image/heif",tif:"image/tiff",tiff:"image/tiff"};
      return m[ext]||"";
    };
    const entries=Object.values(zip.files).filter(x=>!x.dir&&imageMime(x.name));
    for(const entry of entries){
      const data=await entry.async("base64");
      const mime=imageMime(entry.name);
      const key=imageStem(entry.name);
      if(!map.has(key))map.set(key,{key,files:[]});
      map.get(key).files.push({name:entry.name,mimeType:mime,dataUrl:"data:"+mime+";base64,"+data});
    }
    return [...map.values()];
  };
  ...
  const onZip=async(e)=>{
    ...
    try{
      ...
      if(!groups.length)throw new Error("No supported image files found in the ZIP. Common image formats are supported.");
