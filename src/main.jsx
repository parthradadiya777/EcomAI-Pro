...
function ListingAI({product,onBack}){
  ...
  const fillRows=async(imageOnly=false)=>{
    ...
    if(!competitorRefs.length){setError("Analyze the competitor link or upload competitor screenshots first.");return;}
  ...
  return <div className="content">
    ...
    <section className="listing-step-card competitor-reference-card">
      ...
      <div className="reference-actions"><button className="outline" onClick={loadCompetitorReferences} disabled={competitorLoading}>{competitorLoading?<><LoaderCircle size={15} className="spin"/> Reading references…</>:<>Analyze competitor references</>}</button>{competitorRefs.length>0&&<span className="reference-ready"><CheckCircle2 size={15}/> {competitorRefs.length} references ready for all listings</span>}{competitorError&&!competitorScreenshots.length&&<span className="reference-error"><AlertCircle size={15}/> {competitorError}</span>}</div>
      <div className={"competitor-fallback "+(competitorError?"has-error":"")}><div className="fallback-head"><div><strong>{competitorError&&!competitorScreenshots.length?"Link blocked/private — screenshot required":"Screenshot backup (optional)"}</strong><small>{competitorError&&!competitorScreenshots.length?"Upload 1 or more screenshots to continue competitor analysis.":"If the competitor link opens normally, you do not need screenshots. You can still add them for extra reference."}</small></div><label className="upload-mini"><input type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={async e=>{try{await readCompetitorScreenshots(e.target.files)}catch(err){setCompetitorError(err.message)}}}/><ImageIcon size={16}/> {competitorScreenshots.length?"Add more":"Upload screenshots"}</label></div>{competitorScreenshots.length>0&&<div className="competitor-shot-grid">{competitorScreenshots.map((x,i)=><div className="competitor-shot" key={x.name+i}><img src={x.dataUrl} alt={"Competitor screenshot "+(i+1)}/><button type="button" onClick={()=>setCompetitorScreenshots(prev=>prev.filter((_,j)=>j!==i))}>Remove</button><span>Screenshot {i+1}</span></div>)}</div>}{competitorScreenshots.length===0&&<label className="excel-drop compact-drop"><input type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={async e=>{try{await readCompetitorScreenshots(e.target.files)}catch(err){setCompetitorError(err.message)}}}/><ImageIcon size={20}/><strong>Upload Competitor Screenshot(s)</strong><small>1–10 · JPG / PNG / WEBP</small></label>}{competitorScreenshots.length>0&&<span className="reference-ready"><CheckCircle2 size={15}/> {competitorScreenshots.length}/10 screenshots ready</span>}</div>{competitorRefs.length>0&&<div className="reference-preview">{competitorRefs.map((r,i)=><div key={i}><strong>{r.title||"Reference product"}</strong><small>{r.category||r.productType||"Product reference"}{r.color?" · "+r.color:""}{r.fabric?" · "+r.fabric:""}</small></div>)}</div>}
    </section>
