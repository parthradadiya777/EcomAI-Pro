...
createRoot(document.getElementById("root")).render(<App/>)      {generatedPreview.length>0&&<div className="listing-step-card generated-preview-card"><div className="listing-preview-head"><div><span className="eyebrow">STEP 3 · PRODUCT SUMMARY</span><h3>EcomAI master listing ready</h3><p>One ZIP folder = one product. All images inside that folder belong to the same product record.</p></div><span className="row-count">{generatedPreview.length.toLocaleString("en-IN")} products</span></div><div className="content-detection-grid"><div><span>Products</span><b>{generatedPreview.length.toLocaleString("en-IN")}</b></div><div><span>Images grouped</span><b>{imageGroups.reduce((n,g)=>n+(g.files?.length||0),0).toLocaleString("en-IN")}</b></div><div><span>Folder rule</span><b>1 = 1 product</b></div><div><span>Preview</span><b>First 12</b></div></div><div className="generated-product-grid">{generatedPreview.slice(0,12).map((x,i)=><div className="generated-product-card" key={x.sku||i}><div className="generated-product-image">{x.image?<img src={x.image} alt=""/>:<span>NO IMAGE</span>}</div><div className="generated-product-body"><span className="eyebrow">PRODUCT {i+1}</span><h4>{x.sku||"—"}</h4><strong>{x.title||"Needs AI title"}</strong><p>{x.description||"—"}</p><div className="generated-product-meta"><span>Color: {x.color||"—"}</span><span>Keywords: {x.keywords||"—"}</span></div></div></div>)}</div>{generatedPreview.length>12&&<div className="preview-note">Only the first 12 products are shown here. The remaining {generatedPreview.length-12} products are still included in Excel.</div>}<p className="preview-note">This keeps the UI usable even for 5,000+ products.</p></div>}      {generatedPreview.length>0&&<div className="listing-step-card generated-content-card"><div className="listing-preview-head"><div><span className="eyebrow">LISTING CONTENT</span><h3>Title, description & keywords ready</h3><p>EcomAI Pro AI has generated the listing content from your product images and competitor references. Review it here before downloading the EcomAI Pro AI Excel.</p></div><span className="row-count">{generatedPreview.length.toLocaleString("en-IN")} products</span></div><div className="content-detection-grid"><div><span>Titles generated</span><b>{generatedPreview.filter(x=>normalize(x.title)).length}/{generatedPreview.length}</b></div><div><span>Descriptions generated</span><b>{generatedPreview.filter(x=>normalize(x.description)).length}/{generatedPreview.length}</b></div><div><span>Keywords generated</span><b>{generatedPreview.filter(x=>normalize(x.keywords)).length}/{generatedPreview.length}</b></div><div><span>Product images</span><b>{generatedPreview.filter(x=>normalize(x.image)).length}/{generatedPreview.length}</b></div></div></div>}
