...
class AppErrorBoundary extends React.Component{
  ...
  componentDidCatch(error,info){console.error("EcomAI render error",error,info)}
  render(){
    if(this.state.error)return <div style={{minHeight:"100vh",padding:"40px",fontFamily:"Arial,sans-serif",background:"#f8f7fb",color:"#2a2231"}}><h2>EcomAI Pro could not load this screen</h2><p style={{color:"#b42318"}}>{String(this.state.error?.message||this.state.error)}</p><p>Please refresh once. The error has been captured so it can be fixed without another blank screen.</p></div>;
