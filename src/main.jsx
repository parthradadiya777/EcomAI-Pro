...
function ListingAI({product,onBack}){
  ...
  return <div className="content">
    ...
    <section className="pricing-card">\n      <div className="pricing-head">\n        <div><span className="eyebrow">YOUR LISTING PRICE</span><h3>Set your own selling price</h3><p>Seller/customer enters the actual price for this listing. Competitor prices are shown only as research references and never replace your price.</p></div>\n        <span className="pricing-badge">Seller controlled</span>\n      </div>\n      <div className="pricing-input-grid">\n        <label><span>Selling Price <b>*</b></span><div className="price-input"><span>₹</span><input type="number" min="0" value={sellingPrice} onChange={e=>setSellingPrice(e.target.value)} placeholder="Enter your price"/></div></label>\n        <label><span>MRP <small>Optional</small></span><div className="price-input"><span>₹</span><input type="number" min="0" value={mrp} onChange={e=>setMrp(e.target.value)} placeholder="Enter MRP"/></div></label>\n        <label><span>Cost Price <small>Optional</small></span><div className="price-input"><span>₹</span><input type="number" min="0" value={costPrice} onChange={e=>setCostPrice(e.target.value)} placeholder="For margin view"/></div></label>\n      </div>\n      <div className="pricing-reference">\n        <div><span>Observed low</span><b>{priceStats?("₹"+priceStats.low.toLocaleString("en-IN")):"Price unavailable"}</b></div>\n        <div><span>Observed average</span><b>{priceStats?("₹"+priceStats.avg.toLocaleString("en-IN")):"Price unavailable"}</b></div>\n        <div><span>Observed high</span><b>{priceStats?("₹"+priceStats.high.toLocaleString("en-IN")):"Price unavailable"}</b></div>\n      </div>\n      {enteredPrice>0&&<div className="pricing-analysis">\n        <div><span>Your selling price</span><b>{"₹"+enteredPrice.toLocaleString("en-IN")}</b></div>\n        {priceGap!==null&&<div><span>Vs observed average</span><b>{(priceGap>0?"+":"")+priceGap.toFixed(1)+"%"}</b></div>}\n        {discountPct!==null&&<div><span>MRP discount</span><b>{discountPct.toFixed(1)+"%"}</b></div>}\n        {grossMargin!==null&&<div><span>Gross margin before fees</span><b>{"₹"+grossMargin.toLocaleString("en-IN")}</b></div>}\n      </div>}\n      <div className="pricing-note"><ShieldCheck size={14}/> Your entered selling price remains the listing price. EcomAI only adds research context around it.</div>\n    </section>\n    <section className="listing-grid">
