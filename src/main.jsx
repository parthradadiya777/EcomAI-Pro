...
function ListingAI({product,onBack}){
  ...
  return <div className="content">
    ...
    <section className="listing-step-card competitor-reference-card">
      ...
      <div className="reference-actions"><button className="primary analyze-reference-btn" onClick={loadCompetitorReferences} disabled={competitorLoading||(!competitorScreenshots.length&&!competitorUrls.map(x=>normalize(x)).filter(Boolean).length)}>{competitorLoading?<><LoaderCircle size={16} className="spin"/> Analyzing…</>:<><Sparkles size={16}/> Analyze Competitor References</>}</button>{competitorRefs.length>0&&<span className="reference-ready"><CheckCircle2 size={15}/> {competitorRefs.length} references ready for automatic listing generation</span>}{competitorLoading&&<span className="reference-processing"><LoaderCircle size={15} className="spin"/> EcomAI is analyzing competitor screenshots/references…</span>}{competitorError&&<span className="reference-error"><AlertCircle size={15}/> {competitorError}</span>}</div>
    ...
    <section className="listing-workspace">
      ...
      {competitorLoading&&<div className="competitor-processing-modal" role="status" aria-live="polite">
